...
class TestFractalMemory:
    ...
    def test_save_node_state(
        self, memory: "FractalMemory", mock_redis: MagicMock
    ) -> None:
        """Test saving node state to Redis."""
        state = {
            "id": "test-node-1",
            "goal": "Test goal",
            "status": "PENDING",
    ...
    def test_get_node_state_not_found(
        self, memory: "FractalMemory", mock_redis: MagicMock
    ) -> None:
        """Test retrieving non-existent node returns None."""
        mock_redis.get.return_value = None

        result = memory.get_node_state("nonexistent")
    ...
    def test_store_summary(
        ...
        mock_redis.hset.assert_called_once_with(
            "fractal:summaries", "node-1", "This is a summary"
        )
